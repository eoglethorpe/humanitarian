...
class ShelterClusterExtract():
    ...
    def write_to_output(self, row, columns, filename, sheet_name):
        ocolumn = 1
        if self.add_filename:
            self.ows.cell(
                row=self.orow, column=ocolumn,
                value='{}:{}'.format(sheet_name, filename),
            )
            ocolumn = 2
    ...
    def extract_document(self, mrow, user):
        ...
        for row in rws.iter_rows(row_offset=int(header)):
            self.write_to_output(row, columns, filename, sheet_name)
...
if __name__ == '__main__':
    ...
    ShelterClusterExtract(add_filename=False).start('output.xlsx')
